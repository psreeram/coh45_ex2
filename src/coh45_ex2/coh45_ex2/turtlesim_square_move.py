...
class TurtlesimNodeSquareMove(Node):

    def __init__(self):
        super().__init__('turtlesim_node_square_move')   # this name is shown in rqt_graph
    ...
    def timer_callback(self):
        ...
        if (self.i % 5 == 0):  #after every 5 seconds turn by 90 degrees
...
def main(args=None):
    ...
    turtlesim_node_square_move = TurtlesimNodeSquareMove()

    rclpy.spin(turtlesim_node_square_move)

    # Destroy the node explicitly
    # (optional - otherwise it will be done automatically
    # when the garbage collector destroys the node object)
    turtlesim_node_square_move.destroy_node()
    rclpy.shutdown()
